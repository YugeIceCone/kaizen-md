...
def save_store(path: Path, store: dict):
    ...
    # BK-023. Token-efficient ledger in the per-project auto-memory dir
    # so the agent can read backlog state from auto-loaded context
    # instead of shelling `kaizen-backlog list`. Scope is PROJECT
...
def _project_memory_dir() -> Path:
    ...
    rule in scripts/util/better_memory.py::_default_memory_dir:
    KAIZEN_BACKLOG_LEDGER_DIR override → git toplevel slug → cwd slug."""
    env = os.environ.get("KAIZEN_BACKLOG_LEDGER_DIR")
    if env:
    ...
    try:
        ...
        root = (Path(r.stdout.strip()) if r.returncode == 0 and r.stdout.strip()
...
def render_ledger(store: dict) -> str:
    ...
    Header counts + in_flight + next_up + last 10 done. Probe / verify
    bodies are intentionally omitted — read the full item via
    `kaizen backlog show BK-N` on demand."""
    ...
    def _line(it: dict) -> str:
        ...
        return f"- **{it['id']}**  {title}{tag_str}"

    out = ["# Backlog ledger", ""]
    out.append("> Auto-generated by kaizen-backlog. Source of truth: `backlog.json`.")
    out.append("> Use `kaizen backlog show BK-N` to read probe / verify bodies on demand.")
    out.append("")
    ...
    out.append(counts)
    out.append("")
    for sec in ("in_flight", "next_up"):
        rows = by_section[sec]
        out.append(f"## {SECTION_LABELS[sec].lstrip('# ').strip()} ({len(rows)})")
        out.append("")
        if rows:
            for it in rows:
                out.append(_line(it))
            ...
            out.append("_(none)_")
        out.append("")
    done = by_section["done"][-10:]
    out.append(f"## Recently done (last {len(done)})")
    out.append("")
    if done:
        for it in done:
            sha = (it.get("committed") or "")[:9]
            suffix = f"  *(commit `{sha}`)*" if sha else ""
            out.append(f"- **{it['id']}**  {it.get('title','').strip()}{suffix}")
    else:
        out.append("_(none)_")
    out.append("")
    return "\n".join(out)
...
def write_memory_ledger(store: dict) -> Path | None:
    ...
    try:
        ...
        return None  # memory dir unwritable — degrade silently
    target = target_dir / "backlog_ledger.md"
...
def render_md(store: dict) -> str:
    ...
    out.append("> `kaizen backlog {add|start|tick|done|park|render}`.")
...
def cmd_render(store, args, *, json_path: Path, md_path: Path):
    ...
    elif os.environ.get("KAIZEN_BACKLOG_LEDGER_DISABLE") != "1":
        # save_store already writes the memory ledger; `render` bypasses
        # save_store on the common path so wire it explicitly here.
        write_memory_ledger(store)
...
def build_parser():
    ...
    # `done` is a discoverable alias for `tick` — same behavior, friendlier
    # English. (BK-019)
    p_done = sp.add_parser("done", help="alias of tick (mark in_flight → done)")
    p_done.add_argument("id")
    p_done.add_argument("--committed", default=None, help="commit sha (short)")
...
def main():
    ...
    mutating = args.cmd in {"add", "start", "tick", "done", "park", "unpark", "decision"}
    ...
    elif args.cmd in ("tick", "done"): cmd_tick(store, args)

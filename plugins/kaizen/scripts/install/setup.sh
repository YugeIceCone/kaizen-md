...
# DOMAIN-shells Wave C: setup.sh moved to scripts/install/. SKILL_DIR was
# the "skills/workflow/" parent of the historical script dir — many
# siblings (config.py, backlog.py, migrate_paths.sh, _prune_bin_symlinks.py)
# stayed there (deferred DOMAINs). Pin SKILL_DIR to that legacy parent so
# the `$SKILL_DIR/scripts/X` refs below still resolve.
...
for hook_name in pre-commit commit-msg post-commit; do
    # DOMAIN-shells Wave A: git hooks moved to scripts/git-hooks/.
    HOOK_TARGET="$PLUGIN_ROOT/scripts/git-hooks/${hook_name}.sh"

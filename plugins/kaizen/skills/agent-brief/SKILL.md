...
You're in a kaizen-installed session if `kaizen:*` commands appear in your skill catalog (look for `kaizen-backlog`, `kaizen-gatekeeper`, `kaizen-trace`).
...
**Invoke**: gate runs automatically on `git commit`. Dry-run: `bash plugins/kaizen/scripts/git-hooks/pre-commit.sh` (or `kaizen-gatekeeper check --staged` for the Python-side equivalent).
...
| Task | Invocation |
|---|---|
| Dry-run the gate against current diff | `kaizen-gatekeeper check --staged` |

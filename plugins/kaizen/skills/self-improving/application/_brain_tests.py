"""MIGRATION BRIDGE — _brain_tests moved to scripts/self_improving/_brain_tests.py.
...
_CANONICAL = _CANON_DIR / "_brain_tests.py"

for _p in (_LEGACY_DIR, _CANON_DIR):
    if str(_p) not in sys.path:
        sys.path.insert(0, str(_p))

_spec = importlib.util.spec_from_file_location("_brain_tests", _CANONICAL)
_mod = importlib.util.module_from_spec(_spec)
sys.modules["_brain_tests"] = _mod

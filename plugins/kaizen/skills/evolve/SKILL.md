...
After each Phase that writes (Phase 1 entity re-synthesis, Phase 2 belief frontmatter updates, Phase 3 Persona.md update via `promote.js`), call `node ${CLAUDE_PLUGIN_ROOT}/scripts/upstream/schema.js validate <filepath>` on the touched files. Surface its `warnings` (e.g. *"confidence defaulted to 0.5 — review"*) in your final report.
...
3. Run: `node ${CLAUDE_PLUGIN_ROOT}/scripts/upstream/build-index.js --compact` to get the brain index. Use throughout to resolve entities and prevent duplicates.
...
5. Append to evolution log:
   ```bash
   node ${CLAUDE_PLUGIN_ROOT}/scripts/upstream/evolution-log.js CONSOLIDATE "{path} touches=+{n}"
   ```
...
4. If freshness changed OR confidence changed by ≥ 0.05:
   ...
   - Append:
     ```bash
     node ${CLAUDE_PLUGIN_ROOT}/scripts/upstream/evolution-log.js REFLECT "{path} conf {old}→{new} freshness={new_freshness}"
     ```
...
If `--dry-run`:
```bash
node ${CLAUDE_PLUGIN_ROOT}/scripts/upstream/promote.js --dry-run
```

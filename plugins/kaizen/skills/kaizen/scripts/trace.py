...
def cmd_tail(args) -> None:
    # Read only current file + most recent rotated
    files = _all_event_files()
    if not files:
        print("(empty)")
        return
    # Iterate newest-first across the last 1-2 files
    buf = []
    for path in reversed(files[-2:]):
        for ev in _iter_events(path):
            if _matches(ev, src=args.src, evt=args.evt):
                buf.append(ev)
                if len(buf) > args.n * 2:
                    buf = buf[-args.n :]
    # buf may be reverse-chronological-ish, sort by ts
    buf.sort(key=lambda e: e.get("ts", ""))
    for ev in buf[-args.n :]:

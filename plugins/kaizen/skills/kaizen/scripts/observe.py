...
BACKUP_BASE = HOME / ".claude" / "backups" / "kaizen"
...
def cmd_snapshot(name: str = "") -> dict:
    ...
    path = SNAPSHOT_DIR / (fname if fname.endswith(".json") else fname + ".json")
    path.write_text(json.dumps(bundle, indent=2, default=str))
    return {"path": str(path), "composite_hash": bundle["composite_hash"]}

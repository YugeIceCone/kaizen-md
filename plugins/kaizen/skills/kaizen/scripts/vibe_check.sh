#!/usr/bin/env bash
# kaizen vibe-check — augment /kaizen:gate with AI-coding specific checks
...
if [ -x "$SCRIPT_DIR/pre-commit.sh" ]; then
    if bash "$SCRIPT_DIR/pre-commit.sh" >/dev/null 2>&1; then
        echo "  ✓ kaizen:gate dry-run: green" >&2
    else
        echo "  ✗ kaizen:gate dry-run: failed — run /kaizen:gate for details" >&2
    fi
fi

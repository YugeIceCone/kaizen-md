...
Default is dry-run.
  ...
  --remove-backlog     Also delete .workflow/backlog.{json,md}. Default: keep
...
This NEVER touches:
  - .workflow/progress.md, decisions.md, state.json, snapshot.md (project / workflow-routing state)
  - ~/.claude/backups/kaizen/ (always preserved — restore source)
...
echo "  ${GREEN}✓${RESET} ~/.claude/backups/kaizen/$(echo "$REPO" | sed 's|^/||; s|/|-|g')/  (untouched)"
echo "  ${GREEN}✓${RESET} .workflow/{progress.md,state.json,...} (workflow-routing + project state)"
echo ""

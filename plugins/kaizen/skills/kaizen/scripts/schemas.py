...
brain rules, agent-formatting). Full inventory:
  ...
  v1.20.0 additions:
    CodeFile                — `<repo>/.kaizen/onboard.db` rows (codebase index)

  v1.24.0 additions:
    ScrapeItem              — `~/.claude/.kaizen/scrape/index.db` rows (web-scrape index)
...
# skills/kaizen/scripts/onboard_index.py.


VALID_CODE_LANGUAGES = frozenset(
    {
        "rust", "python", "javascript", "typescript", "go", "java", "kotlin",
        "scala", "c", "cpp", "csharp", "swift", "php", "ruby", "shell", "lua",
...
class CodeFile:
    ...
    language: str = ""                            # one of VALID_CODE_LANGUAGES
    bytes: int = 0                                # raw file size
    sloc: int = 0                                 # lines after comment/blank stripping
    snippet: str = ""                             # first ~2KB of cleaned content
    sha: str = ""                                 # 16-hex content sha
    updated_at: str = ""                          # mtime ISO
    id: Optional[int] = None                      # SQLite PK, None pre-insert

    def validate(self) -> list[str]:
        errs: list[str] = []
        if not self.path:
            errs.append("path is required")
        if self.language and self.language not in VALID_CODE_LANGUAGES:
            errs.append(
                f"invalid language={self.language!r} (valid: {sorted(VALID_CODE_LANGUAGES)})"
            )
        if self.bytes < 0:
            errs.append(f"bytes must be >=0, got {self.bytes}")
        if self.sloc < 0:
...
# ─── ScrapeItem (v1.24.0+) ───────────────────────────────────────────
#
# `~/.claude/.kaizen/scrape/index.db` rows. Producer + consumer:
# skills/kaizen/scripts/scrape_index.py. Built from a SmartScraperGraph
...
class ScrapeItem:
    ...
    title: str = ""
    content_json: str = ""                        # raw scrapegraph extraction (JSON)
    text_extract: str = ""                        # denormalized flat text (privacy-filtered)
    sha: str = ""                                 # 16-hex sha1(url|prompt) for de-dup / upsert
    ts: str = ""
    id: Optional[int] = None                      # SQLite PK, None pre-insert
    ...
    def validate(self) -> list[str]:
        ...
        if not self.prompt:
            errs.append("prompt is required (default lives in config.SCRAPE_DEFAULT_PROMPT)")
...
def _self_test() -> None:
    ...
    si = ScrapeItem(
        ...
        prompt="Extract main content as JSON.",
        title="Example",
        content_json='{"title": "Example", "links": []}',
        text_extract="title: Example",
        sha="abc123def4567890",
    )
    assert si.validate() == []
    bad_si = ScrapeItem(url="x")
    errs = bad_si.validate()
    ...
    assert cf.validate() == []
    bad_cf = CodeFile(path="x.rs", language="bogus")
    assert "invalid language" in bad_cf.validate()[0]
    no_path = CodeFile(language="rust")
    assert "path is required" in no_path.validate()[0]

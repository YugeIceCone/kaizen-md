...
The block below conforms to `schemas/agent-formatting.schema.json` (mirrored by the `AgentFormattingSchema` dataclass in `scripts/workflow/schemas.py`). External tools (ajv, jsonschema, future kaizen linters) can validate this block against the JSON Schema directly.

```yaml
# kaizen.agent-formatting.v1 — structured rules
# $schema: ../../schemas/agent-formatting.schema.json
schema_version: 1
rules:
  - id: slash-plain

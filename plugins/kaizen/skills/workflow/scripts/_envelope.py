"""MIGRATION BRIDGE — _envelope moved to scripts/io/_envelope.py.

Bare ``import _envelope`` still resolves here when callers have only
``skills/workflow/scripts/`` on ``sys.path``. This stub loads the
canonical module from ``scripts/io/_envelope.py`` and aliases
``sys.modules["_envelope"]`` to it, so the caller observes the real
...
_CANONICAL = _IO_DIR / "_envelope.py"

for _p in (_LEGACY_DIR, _IO_DIR):
    if str(_p) not in sys.path:
        sys.path.insert(0, str(_p))

_spec = importlib.util.spec_from_file_location("_envelope", _CANONICAL)
_mod = importlib.util.module_from_spec(_spec)
sys.modules["_envelope"] = _mod

"""MIGRATION BRIDGE — _uv moved to scripts/io/_uv.py.

Bare ``import _uv`` still resolves here when callers have only
``skills/workflow/scripts/`` on ``sys.path``. This stub loads the
canonical module from ``scripts/io/_uv.py`` and aliases
``sys.modules["_uv"]`` to it, so the caller observes the real
...
_CANONICAL = _IO_DIR / "_uv.py"

for _p in (_LEGACY_DIR, _IO_DIR):
    if str(_p) not in sys.path:
        sys.path.insert(0, str(_p))

_spec = importlib.util.spec_from_file_location("_uv", _CANONICAL)
_mod = importlib.util.module_from_spec(_spec)
sys.modules["_uv"] = _mod

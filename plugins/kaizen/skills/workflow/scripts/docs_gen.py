...
def main():
    ...
    # `required=False` + `or ["detect"]` fallback so `python3 docs_gen.py`
    # (no args) defaults to listing detected packages. Lets the slash
    # command in commands/docs.md use bare `$ARGUMENTS` without a bash
    # `${VAR:-default}` wrapper (CC's $0/$1 substitution clashes with
    # `bash -c '...' $0` patterns; defaulting in the script avoids that).

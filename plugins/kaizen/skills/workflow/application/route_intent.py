"""MIGRATION BRIDGE — route_intent moved to scripts/workflow/route_intent.py.
...
_CANONICAL = _CANON_DIR / "route_intent.py"

for _p in (_LEGACY_DIR, _CANON_DIR):
    if str(_p) not in sys.path:
        sys.path.insert(0, str(_p))

_spec = importlib.util.spec_from_file_location("route_intent", _CANONICAL)
_mod = importlib.util.module_from_spec(_spec)
sys.modules["route_intent"] = _mod

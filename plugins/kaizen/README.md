...
- **MCP servers** — a single `kaizen` gateway composes 30+ per-domain MCPs via FastMCP `SubServers/mount`. Tool discovery is gated by a `RegexSearchTransform` (curated core always-visible + synthetic `kaizen_search_tools` / `kaizen_call_tool`). Live mount list at `scripts/mcp/gateway.py::SUBSERVERS`.

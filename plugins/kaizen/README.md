...
- **Slash commands** — see `/kaizen:help` for the live catalog (auto-generated from `commands/*.md`). Roots include `/kaizen:setup`, `/kaizen:audit` (+ `:axis`), `/kaizen:workflow`, `/kaizen:observe`, `/kaizen:memory-ledger`, `/kaizen:discovery`, `/kaizen:plugin-development`. Bins like `kaizen-backlog`, `kaizen-brain`, `kaizen-trace`, `kaizen-knowledge`, `kaizen-onboard`, `kaizen-scrape`, `kaizen-models` (Ollama lifecycle) are callable directly from the shell after `/kaizen:setup`.
- **MCP servers** — a single `kaizen` gateway composes 30+ per-domain MCPs via FastMCP `SubServers/mount`. Tool discovery is gated by a `RegexSearchTransform` (curated core always-visible + synthetic `kaizen_search_tools` / `kaizen_call_tool`). Live mount list at `skills/workflow/scripts/gateway.py::SUBSERVERS`.
- **Lifecycle hooks** — registered in `hooks/hooks.json`; covers SessionStart / UserPromptSubmit / PreToolUse / PostToolUse / Stop / PreCompact / SubagentStop / SessionEnd / Notification / InstructionsLoaded / PostCompact / CwdChanged. SessionStart surfaces backlog items; UserPromptSubmit captures input + brain candidates; PreToolUse gates Bash + nudges brain reads; PostToolUse drains inbox + traces tool calls; Stop reminds about in_flight items; PreCompact mines learnings.

...
**IN-PROGRESS 2026-05-12** — workflow-routing build-feature run, schema=hardcoded.
...
- [ ] Create `plugins/kaizen/skills/workflow/scripts/lint_mcp.py`
- [ ] PEP 723 header: `dependencies = ["mcp>=1.0"]` (no torch/sentence-transformers)
- [ ] `FastMCP("kaizen-lint")` instance
- [ ] `_run(cmd, cwd)` helper — subprocess.run wrapper returning `{exit_code, stdout, stderr}`
- [ ] `_repo_root()` helper (mirror state_mcp.py)
- [ ] First tool: `ruff_check(path=".", select="", ignore="", fix=False)` → parses `--output-format=json`
...
- [ ] `ruff_format(path=".", check=True, diff=False)` → `{would_format: [paths], exit_code, raw}`
- [ ] `ruff_rules(category="")` → list of available rule codes (parses `ruff rule --output-format=json` or `ruff linter`)
...
- [ ] `ty_explain(rule="")` → runs `ty explain rule [<name>] --output-format=json`,
      returns parsed JSON
...
- [ ] `lint_path(path)` — fan out to ruff_check + ty_check in sequence,
      return `{ruff: {...}, ty: {...}, summary: {findings_count, severity_max}}`
- [ ] `lint_changed_files(base_ref="HEAD")` — `git diff --name-only --diff-filter=AM <base>`,
...
- [ ] Add `kaizen-lint` entry to `plugins/kaizen/.mcp.json`
- [ ] Add `Bash(uv run --script ${CLAUDE_PLUGIN_ROOT}/skills/workflow/scripts/lint_mcp.py:*)` to plugin.json permissions
- [ ] Refresh plugin cache (`/kaizen:refresh-cache` equivalent script)
- [ ] Run MCP handshake smoke test:

...
| **frontmatter-coverage** | does each `SKILL.md` (a) have `name:` matching its dir basename, AND (b) have ≥3 quoted trigger phrases in `description:` for skill-suggest routing? Catches routing-invisible skills + name drift. Real baseline: 53/90 skills with gaps. | `kaizen-frontmatter` | `frontmatter-coverage` | shipped |
